...
class provide_info_with_order_drink(Action): # "order drink로 추가 정보를 채우는 경우"
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: dict):
        ...
        #정보 끼워넣기
        current_check = False
        print('mesage: ', _orders)
        print('ordere_queue : ',order_queue)
            ...
            if order_queue is not None and len(order_queue) != 0: # 혹시나 이미 주문한걸 찾는지 탐색
                for i,queue_order in enumerate(order_queue):
                    ...
                    for e,val in order.items():
                        if e == 'menu' : continue
                        if val is not None and e in queue_order and val != queue_order[e]:
                            search_key = False
                            break
                        if val is not None and (e not in queue_order or queue_order[e] is None):
...
class change_order(Action):
    ...
    def run(self,dispatcher,tracker,domain):
        ...
        if 'menu' in want_to_cng:
            category = drink_to_category(want_to_cng['menu'])
            if category in ['juice','ade','smoothie']:
                want_to_cng['temp'] = '아이스'
                ordered['temp'] = '아이스'
...
class ask_menu(Action): # 이런 메뉴 파나요?
    ...
    def run(self,dispatcher,tracker,domain):
        ...
        if len(asks) >= 1:
            return [SlotSet("asked_queue",list(asks.values()))]
...
class ask_price(Action): #가격 확인
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        print(asks,skip_group)
        if len(asks) == 0 and skip_group == 0: # no group, no intent -> 내가 주문한 가격을 물어보는 것
            if '사이즈' in latest_user_message or '크기' in latest_user_message:
                dispatcher.utter_message(text=f"기본 사이즈는 스몰이고 미디움 사이즈는 500원 라지 사이즈는 1000원이 추가돼요")
                return []
        ...
        if len(asks) >= 1:
            ...
            print(asked_queue)
            if len(asked_queue) != 0: return [SlotSet("asked_queue",asked_queue)]
...
#         if last_bot_event:
#             last_message = last_bot_event['text']
#             dispatcher.utter_message(text=f"다시 말씀드리겠습니다: {last_message}")
#         else:
#             dispatcher.utter_message(text="이해하지 못했습니다, 다시 입력해 주세요.")
